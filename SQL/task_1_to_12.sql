...
BEGIN
    ...
    IF v_isbooked > 0 THEN
		ROLLBACK;
		SELECT CONCAT("Table ", inputTableNumber, " is already booked on ", inputBookingDate, "- booking cancelled") AS BookingStatus;
    ...
    COMMIT;
		SELECT CONCAT("Booking successful for Table ", inputTableNumber, " on ", inputBookingDate) AS BookingStatus;
	END IF;
END //
